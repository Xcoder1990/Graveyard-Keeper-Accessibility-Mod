...
namespace ConfigurationManager
{
    internal class SettingFieldDrawer
    {
        ...
        private static ConfigurationManagerPlugin _instance;
        ...
        public SettingFieldDrawer(ConfigurationManagerPlugin instance)
        {
            _instance = instance;
        }
        ...
        private static void DrawBoolField(SettingEntryBase setting)
        {
            ...
            var result = GUILayout.Toggle(boolVal, boolVal ? Lang.Get("cm.bool.enabled") : Lang.Get("cm.bool.disabled"), GUILayout.ExpandWidth(true));
            if (result != boolVal)
                setting.Set(result);
        }
        ...
        private static void DrawKeyCode(SettingEntryBase setting)
        {
            ...
            {
                GUILayout.Label(Lang.Get("cm.label.press_any_key"), GUILayout.ExpandWidth(true));
                ...
                if (GUILayout.Button(Lang.Get("cm.button.cancel"), GUILayout.ExpandWidth(false)))
                    _currentKeyboardShortcutToSet = null;
            }
            else
            {
                ...
                if (GUILayout.Button(new GUIContent(Lang.Get("cm.button.set"), null, Lang.Get("cm.button.set_tooltip")), GUILayout.ExpandWidth(false)))
        ...
        private static void DrawKeyboardShortcut(SettingEntryBase setting)
        {
            if (ReferenceEquals(_currentKeyboardShortcutToSet, setting))
            {
                GUILayout.Label(Lang.Get("cm.label.press_any_key_combo"), GUILayout.ExpandWidth(true));
                GUIUtility.keyboardControl = -1;
            ...
            else
            {
                ...
                if (GUILayout.Button(Lang.Get("cm.button.clear"), GUILayout.ExpandWidth(false)))
                {
        ...
        private static void DrawColor(SettingEntryBase obj)
        {
            ...
            GUILayout.BeginVertical();
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    _drawColorHex = GUILayout.Toggle(_drawColorHex, Lang.Get("cm.label.hex"), GUILayout.ExpandWidth(false));
                }
                GUILayout.EndHorizontal();
                GUILayout.BeginHorizontal();
                {
